...
parser = argparse.ArgumentParser(
    description="Report progress for a specific decompiled file"
)
parser.add_argument("name", metavar="name", type=str, help="Name of the original file")
parser.add_argument(
    "count",
    metavar="count",
    type=int,
...
def isIncludeAsm(line):
    return "INCLUDE_ASM" in line


def isPartiallyDecompiled(line):
    return "#ifndef NON_MATCHING" in line


def getLineScore(line):
    if isIncludeAsm(line) == True:
        return 1.0
    if isPartiallyDecompiled(line) == True:
        return -0.25
    return 0.0
